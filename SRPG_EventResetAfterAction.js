 ...
 * Update : now it also stores and resets the "event priorityType data" which is helpfull for mapbattle,
 *          incase you use the SRPG core Version that changes this to display "user" above "target" in mapbattle
 */
 ...
 // Function Overwrite: Replaces SRPG_core.js method of processing at the start of a units action and inserts a line to store move speed and step animation
    Game_Battler.prototype.srpgMakeNewActions = function() {
        ...
        //store event priorityType data
        this._defaultpriorityType = this.event()._priorityType; 
...
// Function Overwrite: Replaces SRPG_core.js method of processing at the end of a units action and inserts a line to reset move speed to be the same as player speed
    ...
    Game_Battler.prototype.onAllActionsEnd = function() {
        if ($gameSystem.isSRPGMode() == true) {
            ...
            // reset Event priorityType if its 3 or 4 which are used to display "user" over "target"
            // the code that handles priorityType 3 or 4 can be found in the srpg core
            // its in the Mapbattle part under "invoke skill effects"
               if  ((this.event()._priorityType === 3) || (this.event()._priorityType === 4)) {
                     this.event()._priorityType = this._defaultpriorityType;
               }
