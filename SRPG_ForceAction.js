 ...
 *
 * And this plugin fixes "Counter not related to skillrange" 
 *
 *===========================================================================================
 ...
 *
 * if that happens while a sv action where user & target are the same as before,
 * this will happen in the same sv battle action.
 * Or else it will trigger actions after the after aktion scene..
 *
 * Thats because sv actions have only 2 battlers and on mapActions this scriptcall will trigger the function:
 * "battler.useMapForceAction(skillID, targetID)" which was shown above..
 *
 *
 *-------------------------------------------------------------------------------------------------------
 *-------------------------------------------------------------------------------------------------------
 ...
 *
 * Known Issue:
 * - on wield counters only the first weapon will use the counterskillID 
 * (for Units with more than 1 weapon better use the weapon skills to define which skill is used)
 *
 * Planed in the next update:
 * - a "combo" skillNote that checks the distance of Units and only trigger another action,
 *   if the unit distance is correct 
 *
 * ===========================================================================================
 * Terms of Use
 * ===========================================================================================
...
Game_Action.prototype.srpgForceActionSetup = function(target) {    
    if (this.item().meta.srpgForceAction) {
        ...
        $gameTemp.srpgForceAction(skillID, userID, targetID);
        return true;
    };
return false;
};
