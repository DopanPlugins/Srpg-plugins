 ...
 *--------------------------------------------------------------------------
 *
 * This Version uses a CommonEvent to trigger the "Pre-ActionPhase". 
 * (this CE = "_BeforeMapBattleCE" ,needs only Script=> 
 * "Scene_Map.prototype.eventBeforeBattle.call(this);" ,in it and nothing else!)
 *
 * The Function "MapActionText" (display SkillNames on Usage)
 * works within a Common Event which should be put in this "Pre-ActionPhase",
 * with the right "if Conditions",.. 
 * ..there is an Example in the Updated-SRPG_Demo for this.
 *
 *
 * I also added "ChangeAniDelaySwitch_ID" & "ChangeAniDelayVarID" ,
 * for more Control over the Delay..this is used for some Skills in the
 * Updated-SRPG_Demo.
 *
 * I did not add the "MapforceAction"- Functions in this Plugin, in order to keep 
 * this Plugin as clean as possible. Pls use the SRPG_MapForceAction.js Plugin ,
 * if you need "MapforceAction"-Functions.
 *
 */
...
//====================================================================
	...
	Scene_Map.prototype.srpgBattleStart = function(userArray, targetArray) {
		...
		$gameTemp.reserveCommonEvent(_BeforeMapBattleCE); //this.eventBeforeBattle();
                // EDIT dopan (this CE = "_BeforeMapBattleCE" ,needs only Script=> "Scene_Map.prototype.eventBeforeBattle.call(this);" ,in it and nothing else!)
	...
	Scene_Map.prototype.update = function() {
		...
		// update map skills_callMapBattle
...
//====================================================================
	...
	// invoke skill effects// case 'useItem'// case 'startcast'
		...
		switch (data.phase) {
			...
			case 'start': //dopan edit added  => " && !action._forcing"
				...
				if ($gameSwitches.value(_changed_Skill_CE_Timing)) {
                                ...
                                user.useItem(action.item()); // Info: this triggers "Custom Execution" if PayCost"true"
