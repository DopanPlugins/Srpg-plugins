 ...
 * @author Dr. Q + アンチョビ (edited Version.. edited by dopan)
 * 
 * @param AoE Color
 ...
 *
 * -dopan edit start-
 * @param OnlyOneTargetInAOE_ID 
 * @desc Switch ID:0 =Switch not used.Switch will be enabled = "true",if there is only 1 target in a AOE.
 * @type switch
 * @default 0 
 * -dopan edit End-
 *
...
(function(){
	...
	var _srpgPredictionWindowMode = Number(coreParameters['srpgPredictionWindowMode'] || 1);
        ...
        var _onlyOneTargetInAoeSwitchID = Number(parameters['OnlyOneTargetInAOE_ID'] || 0); //edit dopan
...
//====================================================================
	...
	Game_Temp.prototype.selectArea = function(user, skill) {
		...
		// there are no targets! //edit dopan Add Switch for usage ,if only 1 target is in AOE
		if (targets.length == 0) return false;
                if (targets.length == 1) { //edit dopan
	            $gameSwitches.setValue(_onlyOneTargetInAoeSwitchID, true) //edit dopan
		} else { //edit dopan
                    $gameSwitches.setValue(_onlyOneTargetInAoeSwitchID, false) //edit dopan
		}; //edit dopan
